...
void drawEgg(){
	...
	float baseVar=1.0/N;
	float u,v;
	float lastU,lastV,lastButOneU,lastButOneV;
	//point tab[10];
	...
	for(int i2=0;i2<=N;i2++){
		for(int i=0;i<=N;i++){
			...
			// cout << i << " " << i2 << endl;
			// cout << u << " " << v << endl;
			// cout << calcX(u,v) << " " << calcY(u,v) << " " << calcZ(u,v) << endl;
			// tab[i,i2]={calcX(u,v),calcY(u,v),calcZ(u,v)};
			// if((i == 0 && i2 == 0) || (i == 1 && i2 == 0 ) || (i == 2 && i2 == 0 )) continue;
			// glColor3f(u, v, u*v);
...
void drawEggV2(){
	...
	// for(int i2=2;i2<N;i2++){
	// 	for(int i=0;i<N;i++){
			glColor3f(tab[i][i2].x,tab[i][i2].y,tab[i][i2].z);
			glVertex3f(tab[i][i2].x,tab[i][i2].y,tab[i][i2].z);
		}
	}
